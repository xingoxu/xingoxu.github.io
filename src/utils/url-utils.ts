...
import { Languages, i18n } from "@i18n/translation";
...
export function getPostSlugWithoutLang(post: CollectionEntry<"posts">) {
  const [lang, ...slug] = post.id.split("/");
  const isForeignPost =
    slug.length > 0 && (Languages as string[]).includes(lang);
  return {
    isForeignPost,
...
export function getPostUrl(post: CollectionEntry<"posts">) {
  const { isForeignPost, id: slug, lang } = getPostSlugWithoutLang(post);
